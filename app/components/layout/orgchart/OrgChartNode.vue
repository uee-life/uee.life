    ...
    <tbody>
      <tr>
       <td :colspan="datasource.children && datasource.children.length ? datasource.children.length*2 : null">
        <div :class="nodeClass" :id="datasource.id" @click.stop="handleClick(datasource)">
      ...
      <template v-if="datasource.children && datasource.children.length">
        ...
        <tr class="nodes">
          <td colspan="2" v-for="child in datasource.children" :key="child.id">
            <node :datasource="child" :handle-click="handleClick" :selected="selected">
...
export default {
  ...
  computed: {
    nodeClass() {
      if(this.datasource.id == this.selected) {
